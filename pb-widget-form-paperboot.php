...
class Widget_Paperboot_Form_Paperboot extends WP_Widget
{
	/*
	...
	*/
	private $email = array(
		'notification' => array(
			'send'    => 'on',
			'to'      => 'ersrgser@dggsr.bvc',
			'from'    => 'ersrgser@dggsr.bvc',
			'reply'   => 'ersrgser@dggsr.bvc',
			'subject' => 'Paperboot submissions',
			'content' => null
		),
		'response' => array(
			'to'      => null,
	...
	*/
	...
	{
		$this->title = __('Paperboot', 'text_domain');
		
		parent::__construct(
			'wpb_widget_pb_form_paperboot', 
			__('PB Paperboot'), 
			array(
	...
	*/
	public function form($instance)
	{
		...
		<div class="pb-widget-form" id="paperboot">
			...
			<div class="message-save message-error"> 
				<p><strong><?php _e('There are no form fields created.', 'text_domain') ?></strong>
					<a href="/wp-admin/post.php?post=&action=edit">Create fields</a>
				</p>
			</div>
			<?php endif ?>
	...
	*/
	...
	{
		$current['post_type'] = 'paperboot';
		extract(wp_parse_args((array) $current, $previous));

		$settings = Paper_Boot_Form::get_pb_settings('paperboot');
