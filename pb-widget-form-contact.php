...
class Widget_Paperboot_Form_Contact extends WP_Widget
{
	/*
	...
	*/
	...
	{
		$this->title = __('Contact', 'text_domain');
		
		parent::__construct(
			'wpb_widget_pb_form_contact', 
			__('PB Contact'), 
			array(
	...
	*/
	public function form($instance)
	{
		...
		<div class="pb-widget-form" id="contact">
			...
			<div class="message-save message-error"> 
				<p><strong><?php _e('There are no form fields created.', 'text_domain') ?></strong>
					<a href="/wp-admin/post.php?post=42&action=edit">Create fields</a>
				</p>
			</div>
			<?php endif ?>
	...
	*/
	...
	{
		$current['post_type'] = 'contact';
		extract(wp_parse_args((array) $current, $previous));

		$settings = Paper_Boot_Form::get_pb_settings('contact');
