...
class Widget_Paperboot_Form_Blog extends WP_Widget
{
	/*
	...
	* @param string
	*/
	public $post_title_field = 'Record';
	...
	*/
	...
	{
		$this->title = __('Blog', 'text_domain');
		
		parent::__construct(
			'wpb_widget_pb_form_blog', 
			__('PB Blog'), 
			array(
	...
	*/
	public function form($instance)
	{
		...
		<div class="pb-widget-form" id="blog">
			...
			<div class="message-save message-error"> 
				<p><strong><?php _e('There are no form fields created.', 'text_domain') ?></strong>
					<a href="/wp-admin/post.php?post=37&action=edit">Create fields</a>
				</p>
			</div>
			<?php endif ?>
	...
	*/
	...
	{
		$current['post_type'] = 'blog';
		extract(wp_parse_args((array) $current, $previous));

		$settings = Paper_Boot_Form::get_pb_settings('blog');
